...
const USB_VENDOR_SUBCLASS: u8 = 0x01;
...
pub struct MyUSBClass<'d, D: Driver<'d>> {
    read_ep: D::EndpointOut,
    write_ep: D::EndpointIn,
}

impl<'d, D: Driver<'d>> MyUSBClass<'d, D> {
    /// Creates a new `MyUSBClass` with the provided UsbBus, number of input and output jacks and `max_packet_size` in bytes.
    ...
    pub fn new(
        ...
        let mut func = builder.function(USB_VENDOR_CLASS, USB_VENDOR_SUBCLASS, PROTOCOL_NONE);

        // Audio control interface
        let mut iface = func.interface();
        let mut alt = iface.alt_setting(USB_VENDOR_CLASS, USB_VENDOR_SUBCLASS, PROTOCOL_NONE, None);
...
/// MyUSB class packet sender.
///
/// You can obtain a `Sender` with [`MyUSBClass::split`]
...
/// MyUSB class packet receiver.
///
/// You can obtain a `Receiver` with [`MyUSBClass::split`]
